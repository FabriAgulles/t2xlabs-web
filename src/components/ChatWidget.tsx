...
const ChatWidget = () => {
  ...
  const sendToWebhook = useCallback(async (message: string): Promise<string> => {
    ...
    try {
      ...
      // Manejo robusto del parsing de JSON
      try {
        ...
        toast.error('Hubo un problema al procesar la respuesta', {
          description: 'Por favor intenta enviar tu mensaje nuevamente',
          duration: 5000,
          position: 'top-right',
        });
      ...
      // Mensaje amigable sin términos técnicos
      let userFriendlyMessage = 'Parece que hay un problema de conexión';

      if (error instanceof Error) {
        // Detectar tipos específicos de errores y dar mensajes más útiles
        if (error.message.includes('tiempo') || error.message.includes('tardó')) {
          userFriendlyMessage = 'La conexión está tardando más de lo normal';
        } else if (error.message.includes('conectar')) {
          userFriendlyMessage = 'No pudimos establecer conexión';
        }
      }
      ...
      setIsReconnecting(false);
      return 'Lo siento, estamos teniendo problemas de conexión. Por favor intenta más tarde.';
    }
  }, [userId, WEBHOOK_URL]);
            ...
            <div className="flex flex-col gap-2">
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  disabled={!inputValue.trim() || inputValue.length > MAX_MESSAGE_LENGTH}
                  size="icon"
                  className="
                    min-w-[44px]
                    min-h-[44px]
              ...
              {/* Badge "Hecho por t2xlabs" con diseño elegante */}
              <div className="flex items-center justify-center gap-1.5 mt-1">
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gradient-to-r from-blue-600 to-cyan-500 shadow-sm hover:shadow-md transition-all duration-300 group">
                  <Sparkles className="w-3 h-3 text-white/90 group-hover:text-white group-hover:rotate-12 transition-all duration-300" />
                  <span className="text-[10px] font-semibold text-white/90 group-hover:text-white transition-colors duration-300">
                    Hecho por{' '}
                    <span className="font-bold">t2xlabs</span>
                  </span>
                </div>
              </div>
            </div>
